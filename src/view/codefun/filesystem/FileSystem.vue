	...
	<div class="file-system full">
		...
		<div class="fs-content-box" id="fcbId">
			...
			<div class="fs-file-box" ref="ffbRef" id="ffbId" v-click-outside="() => current.fileSelected = -1" webkitdirectory>
				<file-shortcut @click.native="current.fileSelected = index" @dblclick.native="dirExplore(item)" @contextmenu.native="fileContextmenuClick" :class="current.fileSelected === index ? 'active':''" :style="'margin: 0 ' + cssConfig.fsiMargin" :key="index" :file="item" :urlSuffix="current.urlSuffix" v-for="(item, index) in current.dirs"/>
			</div>
		</div>
		<div class="fs-status-bar">
			...
			<div class="fs-status-text" v-if="current.fileSelected > -1 && current.dirs[current.fileSelected] && current.dirs[current.fileSelected].type != 'dir'">
				{{current.dirs[current.fileSelected].size}}
			</div>
		</div>
		...
		<div class="fs-message-box" v-if="window.message">
			<div class="fs-mb-template-1">
				<div class="fs-mb-t1-title">
					<i class="iosfont icon-ios-hand" style="font-size: 40px;padding-right: 10px;color: yellowgreen;display: inline;"></i>且慢！！！
				</div>
		...
		<div class="fs-about-box" v-if="window.about" @click.stop="window.about && (window.about = false)">
			<div class="fs-about-box-content fs-mb-template-1">
				...
				<div class="fs-abc-content">
					<div class="fs-abc-c-row bold">版本 v.1.0.1</div>
					<div class="fs-abc-c-row">构建时间 2020-03-06</div>
		...
		.fs-header-box {
			...
			.fs-title-bar {
				...
				.fs-logo {
					...
					background-image: url("../../../assets/img/svg/cloud-file-logo.svg");
				...
				.fs-abc-content {
					...
					.fs-abc-c-row {
						...
						.fs-abc-link {
							...
							&.github {
								background-image: url("../../../assets/img/svg/github-black.svg");
							}

							&.csdn {
								background-image: url("../../../assets/img/svg/csdn.svg");
							}
						}
					}
				}
